...
**Manual `.build-config.json`** — When writing this file via PowerShell, always use `-Encoding ASCII` to avoid the BOM that breaks `JSON.parse` in Node: `Set-Content -Path ".build-config.json" -Value '{"mode":"allraces","races":"all"}' -Encoding ASCII`, then run `npm run build` directly. Useful for a one-off partial rebuild not covered by any `build:<mode>` script (e.g. just `races: "all"` with everything else left passive).

**`npm run build:full` real bug, fixed 2026-09-22 — don't reintroduce it.** `setup-build.js`'s `full` mode used to just delete `.build-config.json`, on the assumption that "no config file" means "rebuild everything." It doesn't: `prebuild.js`'s `isActive()` treats a *missing* config as its own narrower "legacy mode," which only fully rebuilds `drivers`/`seasons` — `races`/`comparisons`/`constructors` were silently restored from whatever stale backup already sat in `out/`, contradicting `full`'s own name and the build-modes table above. Found when a 4-race update (R11–R14) deployed with the race pages still showing old data. Fixed: `full` mode now explicitly writes `{"mode":"full","races":"all","drivers":"all","constructors":"all","seasons":"all","comparisons":"all"}` instead of deleting the file — verify this still holds if `setup-build.js`/`prebuild.js` are ever touched again. **When multiple races land in one update** (more than the single latest), `build:race` isn't enough — it only auto-detects the one most recent completed race. Use `build:full` instead so every affected race/driver/constructor/season/comparison page actually regenerates.
